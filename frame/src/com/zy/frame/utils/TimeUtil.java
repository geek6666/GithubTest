...
//		System.out.println("timeGap: " + timeGap);
    ...
    public static String getBodyOldStr(String time) {
        if (StringUtils.isEmpty(time))
            return "";
        long currentTime = System.currentTimeMillis();
        long timestamp = stringToLong(time, FORMAT_DATE);
        long timeGap = (currentTime - timestamp) / 1000;// 与现在时间相差秒数

        long year = timeGap / YEAR;
        long month = (timeGap - YEAR * year) / MONTH;
        long day = (timeGap - YEAR * year - month * MONTH) / DAY;
    ...
    public static String getDescriptionTimeFromTimestamp2(long timestamp) {
        ...
        String timeStr = null;
        if (timeGap > 3 * DAY) {
            timeStr = longToString(timestamp, FORMAT_DATE_TIME_SECOND);
        } else if (timeGap > DAY) {// 1天以上
            timeStr = timeGap / DAY + "天前";
        } else if (timeGap > HOUR) {// 1小时-24小时
            timeStr = timeGap / HOUR + "小时前";
